...
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { ChevronLeft, ChevronRight, RotateCcw, Home, Mic, Sparkles, Volume2 } from 'lucide-react';
import Layout from '@/components/Layout/Layout';
import WordDisplay from '@/components/Therapy/WordDisplay';
import AudioControls from '@/components/Therapy/AudioControls';
import PhonemeBreakdown from '@/components/Therapy/PhonemeBreakdown';
...
export default function SpeechTherapy() {
  ...
  // Called by AudioControls when AI feedback is ready → auto-play
  const handleFeedbackReady = useCallback((text: string) => {
    setFeedbackText(text);
    playFeedbackTTS(text); // auto-play immediately
  }, []);
  ...
  const playFeedbackTTS = (text: string) => {
    ...
    utter.pitch = 1.2;      // slightly higher/friendlier pitch
    utter.volume = 1;
    ...
    <Layout>
      <div className="min-h-full bg-background">
        ...
        <div className="relative z-10 p-6 md:p-8 max-w-5xl mx-auto">
            ...
            <div className="flex justify-center gap-1.5 mt-3">
              {therapyWords.map((w, i) => (
                <button key={i} onClick={() => setCurrentIndex(i)}
                  className={`transition-all duration-200 rounded-full ${i === currentIndex ? 'w-5 h-2.5 bg-primary' : recordings[therapyWords[i].id] ? 'w-2.5 h-2.5 bg-emerald-400' : 'w-2.5 h-2.5 bg-muted/40 hover:bg-muted'}`}
                />
              ))}
            </div>
          </div>
          ...
          {/* Main Card */}
          <AnimatePresence mode="wait">
            <motion.div key={word.id} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.3 }}
              className="bg-white/80 backdrop-blur-xl border border-white/60 shadow-2xl shadow-black/5 rounded-3xl p-8 md:p-10"
            >
              <div className="grid md:grid-cols-2 gap-10 items-start">
                <div className="flex flex-col items-center">
