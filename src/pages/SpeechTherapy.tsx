...
import { ChevronLeft, ChevronRight, RotateCcw, Home, Mic, Sparkles, Volume2, Loader2 } from 'lucide-react';
...
export default function SpeechTherapy() {
  ...
  const therapyWords: SpeechTherapyWord[] = therapyData?.data || [];
  const word = therapyWords[currentIndex];

  const currentRecording = word ? recordings[word._id!] : null;
  const hasRecording = !!currentRecording;
  const progressPct = therapyWords.length > 0 ? ((currentIndex + 1) / therapyWords.length) * 100 : 0;
  const completedCount = Object.keys(recordings).length;

  const handleRecordingComplete = useCallback(() => {
    if (word) {
      setRecordings(prev => ({ ...prev, [word._id!]: word.mockResponse }));
    }
  }, [word]);
  ...
  const replayFeedback = () => {
    if (feedbackText) playFeedbackTTS(feedbackText);
  };
  ...
  }, [currentIndex, therapyWords.length]);
  ...
  const resetCurrent = useCallback(() => {
    if (word) {
      setRecordings(prev => { const n = { ...prev }; delete n[word._id!]; return n; });
      setSpeechPronunciationScore(null);
      setFeedbackText(null);
      window.speechSynthesis.cancel();
    }
     ...
     return (
      <Layout>
        <div className="min-h-screen flex flex-col items-center justify-center gap-4">
          <p className="text-muted-foreground">No therapy words found.</p>
        ...
        <div className="relative z-10 p-6 md:p-8 max-w-5xl mx-auto">
          ...
          <div className="mb-7">
            ...
            <div className="w-full h-2.5 bg-muted/20 rounded-full overflow-hidden">
              <motion.div animate={{ width: `${progressPct}%` }} transition={{ duration: 0.5 }} className="h-full rounded-full bg-gradient-to-r from-primary to-primary-soft" />
            </div>
            <div className="flex justify-center gap-1.5 mt-3">
              {therapyWords.map((w, i) => (
                <button key={i} onClick={() => setCurrentIndex(i)}
                  className={`transition-all duration-200 rounded-full ${i === currentIndex ? 'w-5 h-2.5 bg-primary' : recordings[therapyWords[i]._id!] ? 'w-2.5 h-2.5 bg-emerald-400' : 'w-2.5 h-2.5 bg-muted/40 hover:bg-muted'}`}
                />
              ))}
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={word._id} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.3 }}
                ...
                <div className="space-y-6">
                  ...
                  {hasRecording && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-center">
                      <Button variant="ghost" size="sm" onClick={resetCurrent} className="gap-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted/30">
          ...
          <div className="flex justify-between items-center mt-6">
            ...
            {currentIndex === therapyWords.length - 1 && hasRecording ? (
              ...
              <Button onClick={goNext} disabled={currentIndex === therapyWords.length - 1}
